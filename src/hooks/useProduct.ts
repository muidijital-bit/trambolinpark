...
export function useProduct(slug: string | undefined) {
  ...
  useEffect(() => {
    ...
    function loadProduct(data: Record<string, unknown>) {
      const p: Product = { id: (data.slug ?? data.id) as string, title: data.title as string, description: data.description as string, imageUrl: data.image_url as string, category: data.category as string, categoryName: data.category_name as string, features: (data.features as string[]) ?? [] };
      setProduct(p);
