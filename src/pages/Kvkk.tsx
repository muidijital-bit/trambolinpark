          ...
          {sections.map((s, i) => (
            <div key={i} className={i < sections.length - 1 ? 'mb-4 pb-4 border-bottom' : ''}>
              <h2 className="d-flex align-items-center gap-2 fw-black mb-2" style={{ fontSize: 16, color: '#1a1a1a' }}>
                <span className="d-flex align-items-center justify-content-center rounded-3 flex-shrink-0" style={{ width: 32, height: 32, background: 'rgba(159,201,26,.1)', color: '#9fc91a' }}><FileText size={14} /></span>
                {s.title}
              </h2>
              <p className="mb-0 text-secondary" style={{ fontSize: 14, lineHeight: 1.75, paddingLeft: 44 }}>{s.body}</p>
            </div>
          ))}
          <div className="mt-4 p-4 rounded-3 d-flex align-items-start gap-3" style={{ background: '#f8fafc', border: '1px solid #e2e8f0' }}>
            <span className="d-flex align-items-center justify-content-center rounded-3 flex-shrink-0" style={{ width: 40, height: 40, background: 'rgba(159,201,26,.1)', color: '#9fc91a' }}><Lock size={18} /></span>
            <div>
